...
//                      Collectors.toMap(Meal::getDate, Meal::getCalories, Integer::sum)
    ...
    private static MealTo createWithExcess(Meal meal, boolean excess) {
        MealTo newMealTo = new MealTo(meal.getId(), meal.getDateTime(), meal.getDescription(), meal.getCalories(), excess);
        newMealTo.setId(meal.getId());
        return newMealTo;
    }

    public static List<MealTo> getAllMeals(List<Meal> mealList, int caloriesPerDay) {
        return getFilteredWithExcessInOnePass(mealList, LocalTime.MIN, LocalTime.MAX, caloriesPerDay);
    }
}